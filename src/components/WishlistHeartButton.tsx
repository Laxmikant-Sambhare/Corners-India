...
import { discoverHeartIconSize } from "../navDesignTokens";
import {
  isProductInWishlist,
  useWishlistStore,
...
export function WishlistHeartButton({ product, size = discoverHeartIconSize }: Props) {
  const items = useWishlistStore((s) => s.items);
  const toggleWishlist = useWishlistStore((s) => s.toggleWishlist);
  const saved = isProductInWishlist(items, product);
